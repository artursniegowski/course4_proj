...
class OmdbMovie:
    """A simple class to represent move data coming back from  OMDB
    ...
    def runtime_minutes(self):
        self.check_for_detail_data_key(self.data["Runtime"])
...
class OmdbClient:
    ...
    def get_by_imdb_id(self, imdb_id):
        """Get a movie by its IMBD ID"""
        logger.info("Fetching detail for IMDB ID %s", imdb_id)
        res = self.make_request({"i": imdb_id})
        return OmdbMovie(res.json())
    
    def search(self, search):
        """Search for movies by title. This is a generator so all results from all 
        pages will be iterated across."""
        page = 1
        seen_results = 0
        total_results = None
        ...
        while True:
            ...
            res = self.make_request({"s": search, "type": "movie", "page": str(page)})
            res_body = res.json()
